    ...
    def post(self, request):
        ...
        if basket:
            ...
            if int(discount) == 0 and delivery:
                save_invoice = Invoice.objects.create(total_price= float(gross_amount),discounted_amount=float(net_total),status=status,delivery_charges=delivery,recieved_cash=recieved_cash,returned_cash=returned_cash,delivery_man_name=delivery_man)
            elif delivery and int(discount) > 0:
                save_invoice = Invoice.objects.create(total_price= float(gross_amount),discounted_amount=float(net_total),discount=discount,status=status,delivery_charges=delivery,recieved_cash=recieved_cash,returned_cash=returned_cash,delivery_man_name=delivery_man)
